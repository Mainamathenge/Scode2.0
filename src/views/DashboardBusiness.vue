  ...
  <div>
    <v-container>
      ...
      <v-row :class="{ 'mx-1 ': $vuetify.breakpoint.mdAndUp }" class="mt-8">
        <v-col>
          <BusinessesTable
            :businesses="receivedBusinesses"
            :businessesObj="businessesObj"
            @select-business="setBusiness"
          />
        </v-col>
      </v-row>
    </v-container>
    <v-snackbar v-model="snackbar" :right="$vuetify.breakpoint.lgAndUp">
      You have selected {{ selectedBusiness.name }}
...
export default {
  ...
  methods: {
    setBusiness(event) {
      this.snackbar = true;
      this.selectedBusiness.name = event.name;
    },
    logout() {
